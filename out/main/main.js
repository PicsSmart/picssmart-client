...
function _vaultPath(options2) {
  ...
  if (fs$2.existsSync(possibleVaultPath)) {
...
function configDotenv(options2) {
  ...
  try {
    const parsed = DotenvModule.parse(fs$2.readFileSync(dotenvPath, { encoding }));
    let processEnv = process.env;
    if (options2 && options2.processEnv != null) {
      processEnv = options2.processEnv;
    }
...
const FormData$3 = /* @__PURE__ */ getDefaultExportFromCjs(form_data);
...
electron.app.whenReady().then(() => {
  ...
  electron.ipcMain.handle("dialog:zipFolder", async (_, sourceFolder) => {
    ...
    const folderName = require$$1__namespace.basename(sourceFolder);
